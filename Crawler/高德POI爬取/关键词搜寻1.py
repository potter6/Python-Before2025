...
if __name__ == '__main__':
    ...
    # keyword = ["停车场","公交车站","地铁站","体育休闲服务","公园内部设施","交通设施服务"]
    # keyword = ["公交车站"]
    ...
    key='e7d3e7cf12933eb1a8d0e48117cc5bcc'
    ...
    for i in range(0, len(city)):
        ...
        for j in range(0, len(type)):
            ...
            bkeys = ['id', 'biz_type', 'name', 'type', 'address', 'tel', 'location', 'pcode', 'pname', 'citycode',
                    'cityname','adcode', 'adname', 'business_area','rating']
            # 写入数据到json文件，第二次运行可注释
            getPOIdata(page_size, json_name, url_amap)
            # 读取json文件数据写入到excel
            # os.makedirs("data_index\\"+city[i])
            write_data_to_excel(json_name, hkeys, bkeys, "out\\" + city[0] + type[j] + "-高德地图")
            if (i % 13 == 0):
               time.sleep(15)
            elif (i % 13 != 0):
               time.sleep(15)