...
def get_report_url_list(page, limit):
    format_params = f"page={page}&limit={limit}&taxonid=&startTime=&endTime=&province=上海&city=上海&district=&pointname=&username=&serial_id=&ctime=&taxonname=&state=&mode=0&outside_type=0"
...
def get_all_report_url_list():
    _data_list = []
    with open(r"SpiderChaser-main\bird-report\aid.txt", "r", encoding="utf-8") as _f:
        lines = _f.readlines()
        for line in lines:
    ...
    while page <= 83:
        try:
            ...
            continue
        page += 1
    with open(r"bird-report\aid.txt", "w", encoding="utf-8") as _f:
...
if __name__ == '__main__':
    ...
    with open(r"SpiderChaser-main\bird-report\jQuertAjax.js", "r", encoding="utf-8") as f:
        node_path = r"d:\Productivity Tools\Node\node_modules"
