...
# 被发送的邮箱名和内容
yag.send(to=emailname, subject="开始", contents="爬取文本信息\n"+"运行时间"+f"{str(datetime.datetime.now())}")
###############################################只需要修改filename即可，确定好datainputpath和dataoutputpath这两个文件夹与该py文件的相对位置
filename='20211101_20211201 35651 .csv'
###############################################注意命名格式是否正确 与csv的名字对应好
datainputpath='/kaggle/input/aminuosi/'
dataoutputpath='/kaggle/working/'
###############################################设置好的输入输出文件所在文件夹路径
...
# 数据输入 2 装有key的csv文件
key=pd.read_csv('/kaggle/input/aminuosi/YangBaiduAccesskey.csv')
# key的读取 
...
def process_row(row):
    try:
        ...
        # if pd.isna(row['baidu_confidence']):
            ...
            # 休息几秒钟 确保正确跑完 
            # sleep(0.04*random.randint(2,3)) # 或者这样0.04秒到0.06秒 之间随机休息比较好
            ##########################################################################################################
...
yag.send(to=emailname, subject="结束", contents=filename+"爬取文本信息\n"+"运行时间"+f"{str(datetime.datetime.now())}")
